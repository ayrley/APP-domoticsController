    ...
    m_onWakeRequest(std::move(onWakeRequest))
{
    ...
    m_titleLabel = new nanogui::Label(this, "D.O.M.E.", "sans-bold");
    m_titleLabel->set_font_size(38);
    m_titleLabel->set_color(nanogui::Color(0, 235, 255, 255));

    m_subtitle = new nanogui::Label(this, " Domotics Operations & Monitoring Engine", "sans");
    m_subtitle->set_font_size(22);
    m_subtitle->set_color(nanogui::Color(130, 215, 225, 255));
...
void ScreensaverPage::perform_layout(NVGcontext *ctx)
{
    ...
    const nanogui::Vector2i subtitleSize = m_subtitle->preferred_size(ctx);
    ...
    m_subtitle->set_position(nanogui::Vector2i(
        centerX - subtitleSize.x() / 2,
        centerY));
    constexpr int kBottomMargin = 28;
    m_hintLabel->set_position(nanogui::Vector2i(
        centerX - hintSize.x() / 2,
        m_size.y() - hintSize.y() - kBottomMargin));
}
